...
var fetchRealPrice = async (runtime2) => {
  const http = new cre.capabilities.HTTPClient;
  const consensusExecution = await http.sendRequest(runtime2, (sendRequester) => {
    ...
    const price2 = Number(data.ethereum.usd);
  ...
  runtime2.log(`[DEBUG] Real price consensus result: ${price}`);
  if (!Number.isFinite(price)) {
    throw new Error("Invalid price returned from consensus");
  }
  return price;
};
async function fetchControlledPrice(runtime2) {
  const http = new cre.capabilities.HTTPClient;
  const consensusExecution = await http.sendRequest(runtime2, (sendRequester) => {
    const response = sendRequester.sendRequest({
    ...
    if (!ok(response))
      throw new Error("HTTP request failed");
    const data = json(response);
    const price2 = Number(data.price);
    if (!Number.isFinite(price2)) {
      throw new Error("Invalid price in HTTP response");
    }
    return price2;
  ...
  const price = Number(consensusExecution.result());
  runtime2.log(`[DEBUG] Controlled price consensus result: ${price}`);
  if (!Number.isFinite(price)) {
    throw new Error("Invalid price returned from consensus");
  }
  return price;
...
function advancedConsensus(realPrice, controlledPrice) {
  ...
  const deviation = Math.abs(realPrice - controlledPrice) / realPrice * 100;
...
function computeSystemicRisk(vaultStates, volatilityStdDev, reserveHealth) {
  ...
  const weightedDebt = unhealthy.reduce((sum, v) => sum + Number(v.debt), 0);
  const totalDebt = vaultStates.reduce((sum, v) => sum + Number(v.debt), 0);
  ...
  const reserveStress = reserveHealth < 100 ? (100 - reserveHealth) / 20 : 0;
  const weightUnhealthy = 0.4;
  const weightVolatility = 0.25;
  const weightReserve = 0.15;
  const weightDebt = 0.2;
  const rawScore = unhealthyRatio * weightUnhealthy + volatilityFactor * weightVolatility + reserveStress * weightReserve + debtStress * weightDebt;
...
function computeVolatility(prices, threshold = 0.05) {
  if (prices.length === 0) {
    return {
      mean: 0,
      variance: 0,
...
async function getVaultState(runtime2, selector, vault, price) {
  const evmVault = new cre.capabilities.EVMClient(BigInt(selector));
  const evmExecutor = new cre.capabilities.EVMClient(BigInt(runtime2.config.chainB.selector));
  ...
  const [debtResp, healthResp, escalationResp, nonceResp] = await Promise.all([
    evmVault.callContract(runtime2, {
      call: encodeCallMsg({
        from: zeroAddress,
        to: vault,
        data: debtCall
    ...
    evmExecutor.callContract(runtime2, {
      call: encodeCallMsg({
        from: zeroAddress,
        to: runtime2.config.chainB.executor,
        data: nonceCall
      }),
      blockNumber: LAST_FINALIZED_BLOCK_NUMBER
    }).result()
...
async function getStableSupply(runtime2, selector, stableAddress) {
  ...
  const response = await evm.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: stableAddress,
      data: supplyCall
...
function fromTwos(_value, _width) {
  ...
  assert2(value2 >> width === BN_0, "overflow", "NUMERIC_FAULT", {
    operation: "fromTwos",
    fault: "overflow",
    value: _value
  });
  if (value2 >> width - BN_1) {
...
function toNumber(value2) {
  return getNumber(toBigInt(value2));
}
...
class Result extends Array {
        ...
        if (typeof prop === "string") {
          ...
          if (value2 instanceof Function) {
            return function(...args2) {
              return value2.apply(this === receiver ? target : this, args2);
            };
  ...
  getValue(name) {
    ...
    const value2 = this[index];
    if (value2 instanceof Error) {
      throwError(`property ${JSON.stringify(name)}`, value2.error);
    }
    return value2;
  }
...
class Coder {
  ...
  _throwError(message, value2) {
    assertArgument(false, message, this.localName, value2);
  }
}
...
class Writer {
  ...
  writeValue(value2) {
    return this.#writeData(getValue(value2));
  }
  writeUpdatableValue() {
    ...
    return (value2) => {
      this.#data[offset] = getValue(value2);
    };
  }
}
...
function setBigUint64(view, byteOffset, value2, isLE3) {
...
var bitSet = (n, pos, value2) => {
  return n | (value2 ? _1n3 : _0n3) << BigInt(pos);
};
...
class Signature {
  ...
  clone() {
    const clone2 = new Signature(_guard2, this.r, this._s, this.v);
    if (this.networkV) {
...
var _gaurd = {};
function n(value2, width) {
  let signed = false;
  if (width < 0) {
    signed = true;
    width *= -1;
  }
  return new Typed(_gaurd, `${signed ? "" : "u"}int${width}`, value2, { signed, width });
}
function b(value2, size2) {
  return new Typed(_gaurd, `bytes${size2 ? size2 : ""}`, value2, { size: size2 });
}
...
class Typed {
  ...
  #options;
  _typedSymbol;
  constructor(gaurd, type, value2, options) {
    if (options == null) {
      options = null;
    }
    assertPrivate(_gaurd, gaurd, "Typed");
    defineProperties(this, { _typedSymbol, type, value: value2 });
  ...
  static from(type, value2) {
    return new Typed(_gaurd, type, value2);
  }
  static uint8(v) {
    return n(v, 8);
  }
  ...
  static isTyped(value2) {
    return value2 && typeof value2 === "object" && "_typedSymbol" in value2 && value2._typedSymbol === _typedSymbol;
  }
  static dereference(value2, type) {
    if (Typed.isTyped(value2)) {
      if (value2.type !== type) {
        throw new Error(`invalid type: expecetd ${type}, got ${value2.type}`);
      }
      return value2.value;
    }
    return value2;
...
class AddressCoder extends Coder {
  ...
  encode(writer, _value) {
    let value2 = Typed.dereference(_value, "string");
    try {
...
class ArrayCoder extends Coder {
  ...
  encode(writer, _value) {
    const value2 = Typed.dereference(_value, "array");
    if (!Array.isArray(value2)) {
    ...
    assertArgumentCount(value2.length, count, "coder array" + (this.localName ? " " + this.localName : ""));
    let coders = [];
    for (let i2 = 0;i2 < value2.length; i2++) {
...
class BooleanCoder extends Coder {
  ...
  encode(writer, _value) {
    const value2 = Typed.dereference(_value, "bool");
...
class NumberCoder extends Coder {
  ...
  encode(writer, _value) {
    let value2 = getBigInt(Typed.dereference(_value, this.type));
    let maxUintValue = mask(BN_MAX_UINT256, WordSize * 8);
    if (this.signed) {
      let bounds = mask(maxUintValue, this.size * 8 - 1);
      if (value2 > bounds || value2 < -(bounds + BN_13)) {
        this._throwError("value out-of-bounds", _value);
      }
      value2 = toTwos(value2, 8 * WordSize);
    } else if (value2 < BN_04 || value2 > mask(maxUintValue, this.size * 8)) {
      this._throwError("value out-of-bounds", _value);
    }
    return writer.writeValue(value2);
  }
  decode(reader) {
    let value2 = mask(reader.readValue(), this.size * 8);
    if (this.signed) {
      value2 = fromTwos(value2, this.size * 8);
    }
    return value2;
  }
}
...
class TupleCoder extends Coder {
  ...
  constructor(coders, localName) {
    ...
    coders.forEach((coder) => {
      ...
      types4.push(coder.type);
    });
    const type = "tuple(" + types4.join(",") + ")";
  ...
  encode(writer, _value) {
    const value2 = Typed.dereference(_value, "tuple");
    ...
    if (type) {
      ...
      if (type === "OPEN_PAREN") {
        ...
        if (tokens.length > 0 && tokens[tokens.length - 1].type === "NUMBER") {
          const value2 = tokens.pop().text;
          suffix = value2 + suffix;
          tokens[tokens.length - 1].value = getNumber(value2);
        }
...
class ParamType {
  ...
  walk(value2, process2) {
    if (this.isArray()) {
      if (!Array.isArray(value2)) {
        throw new Error("invalid array value");
      }
      if (this.arrayLength !== -1 && value2.length !== this.arrayLength) {
        throw new Error("array is wrong length");
      }
      const _this = this;
      return value2.map((v) => _this.arrayChildren.walk(v, process2));
    }
    if (this.isTuple()) {
      if (!Array.isArray(value2)) {
        throw new Error("invalid tuple value");
      }
      if (value2.length !== this.components.length) {
        throw new Error("array is wrong length");
      }
      const _this = this;
      return value2.map((v, i2) => _this.components[i2].walk(v, process2));
    }
    return process2(this.type, value2);
  }
  #walkAsync(promises, value2, process2, setValue) {
    if (this.isArray()) {
      if (!Array.isArray(value2)) {
        throw new Error("invalid array value");
      }
      ...
      const result2 = value2.slice();
      result2.forEach((value3, index) => {
        childType.#walkAsync(promises, value3, process2, (value4) => {
          result2[index] = value4;
        });
      });
      setValue(result2);
      return;
    ...
    if (this.isTuple()) {
      ...
      if (Array.isArray(value2)) {
        ...
        result2 = components.map((param) => {
          ...
          if (!(param.name in value2)) {
            throw new Error(`missing value for component ${param.name}`);
          }
          return value2[param.name];
        });
      }
      ...
      result2.forEach((value3, index) => {
        components[index].#walkAsync(promises, value3, process2, (value4) => {
          result2[index] = value4;
        });
      });
      setValue(result2);
    ...
    const result = process2(this.type, value2);
    if (result.then) {
  ...
  async walkAsync(value2, process2) {
    const promises = [];
    const result = [value2];
    this.#walkAsync(promises, value2, process2, (value3) => {
      result[0] = value3;
    });
    if (promises.length) {
  ...
  static isParamType(value2) {
    return value2 && value2[internal] === ParamTypeInternal;
  }
}
...
class AbiCoder {
  ...
  getDefaultValue(types4) {
    const coders = types4.map((type) => this.#getCoder(ParamType.from(type)));
    const coder = new TupleCoder(coders, "_");
    return coder.defaultValue();
  }
  encode(types4, values) {
    assertArgumentCount(values.length, types4.length, "types/values length mismatch");
    const coders = types4.map((type) => this.#getCoder(ParamType.from(type)));
    const coder = new TupleCoder(coders, "_");
  ...
  decode(types4, data, loose) {
    const coders = types4.map((type) => this.#getCoder(ParamType.from(type)));
    const coder = new TupleCoder(coders, "_");
    return coder.decode(new Reader(data, loose, defaultMaxInflation));
  }
  static _setDefaultMaxInflation(value2) {
    assertArgument(typeof value2 === "number" && Number.isInteger(value2), "invalid defaultMaxInflation factor", "value", value2);
    defaultMaxInflation = value2;
  }
...
async function reportPoR(runtime2, reserves, supply, riskScore) {
  const selectorA = BigInt(runtime2.config.chainA.selector);
  const authority = runtime2.config.chainA.authority;
  const payload = AbiCoder.defaultAbiCoder().encode(["uint256", "uint256", "uint256"], [reserves, supply, BigInt(riskScore)]);
  const reportRequest = prepareReportRequest(`0x${payload.slice(2)}`);
  const report2 = await runtime2.report(reportRequest, {
...
async function getReserveHealth(runtime2) {
  const custodians = runtime2.config.custodians;
  ...
  const http = new cre.capabilities.HTTPClient;
  const execution = await http.sendRequest(runtime2, (sendRequester) => {
    const response = sendRequester.sendRequest({
      url: "http://localhost:3001/reserve",
      method: "GET"
    }).result();
    if (!ok(response))
      throw new Error("Reserve HTTP request failed");
    const data = json(response);
  ...
  const value2 = await execution().result();
...
function getNextNonce(current) {
  return current + 1n;
}
async function routeExecution(runtime2, action, vaultState, price) {
  const selectorB = BigInt(runtime2.config.chainB.selector);
  const executorAddress = runtime2.config.chainB.executor;
  const vault = vaultState.vault;
  const nonce = getNextNonce(vaultState.vaultNonce);
  logger.info("Nonce verification " + JSON.stringify({
    vault,
    chainNonce: vaultState.vaultNonce.toString(),
    nextNonceUsed: nonce.toString()
  }));
  logger.info("Execution started" + JSON.stringify({
    action,
    vault,
  ...
  try {
    ...
    logger.info("Report submitted to DON", JSON.stringify({
      action,
      vault,
...
async function runWorkflow(runtime2) {
  runtime2.log("----- SentinelX CRE Workflow Starting -----");
  const [realPrice, controlledPrice] = await Promise.all([
    fetchRealPrice(runtime2),
    fetchControlledPrice(runtime2)
  ]);
  const consensus = advancedConsensus(realPrice, controlledPrice);
  if (!consensus.valid) {
    runtime2.log(`Consensus rejected. Deviation: ${consensus.deviation}%`);
    runtime2.log("⚠️ TEST MODE: Forcing execution despite invalid consensus");
  }
  const price = consensus.valid ? consensus.agreedPrice : realPrice;
  runtime2.log(`Consensus agreed price: ${price}`);
  const reserveHealth = await getReserveHealth(runtime2);
  let reserveAction = "NONE";
  if (reserveHealth < 90)
    reserveAction = "EMERGENCY";
  else if (reserveHealth < 95)
    reserveAction = "FREEZE";
  else if (reserveHealth < 100)
    reserveAction = "RESTRICT";
  runtime2.log(`Reserve health: ${reserveHealth}% | Action=${reserveAction}`);
  let freezeTriggered = false;
  if (reserveAction === "EMERGENCY") {
    runtime2.log("\uD83D\uDEA8 EMERGENCY reserve breach. Freezing all vaults.");
    freezeTriggered = true;
  } else if (reserveAction === "FREEZE") {
    runtime2.log("⚠ Reserve critical. Freezing system.");
    freezeTriggered = true;
  }
  const vaultStates = [];
  for (const vault of runtime2.config.chainA.vaults) {
    const state = await getVaultState(runtime2, runtime2.config.chainA.selector, vault, price);
    const readableHealth = Number(state.health) / 1e8;
    runtime2.log(`[Vault ${vault}] Health=${readableHealth.toFixed(2)}% (raw=${Number(state.health)}) | Escalation=${EscalationOrder[state.lastEscalation]} | Nonce=${state.vaultNonce}`);
    vaultStates.push({
      vault,
      ...state,
  ...
  const systemic = computeSystemicRisk(vaultStates, volatility.stdDev, reserveHealth);
  runtime2.log(`Systemic Risk Score=${systemic.score.toFixed(2)} | Freeze=${systemic.systemicFreeze}`);
  const systemHealthIndex = ((1 - systemic.score) * 100).toFixed(1);
  runtime2.log("=================================================");
  runtime2.log(`System Health Index = ${systemHealthIndex}/100`);
  runtime2.log("=================================================");
  let systemLevel = "STABLE";
  if (systemic.score > 0.7)
    systemLevel = "CRITICAL";
  else if (systemic.score > 0.5)
    systemLevel = "ELEVATED";
  runtime2.log(`System Health Index=${systemHealthIndex}/100 | Risk Level=${systemLevel}`);
  runtime2.log("Preparing PoR report for ReserveAuthority...");
  const supply = await getStableSupply(runtime2, runtime2.config.chainA.selector, runtime2.config.chainA.stable);
  const reserves = BigInt(Math.floor(reserveHealth * 10000000000000000));
  const riskScorePercent = Math.floor(systemic.score * 100);
  await reportPoR(runtime2, reserves, supply, riskScorePercent);
  runtime2.log(`PoR Report Sent → Reserves=${reserves.toString()} | Supply=${supply.toString()} | RiskScore=${riskScorePercent}%`);
  freezeTriggered = freezeTriggered || systemic.systemicFreeze;
  if (freezeTriggered) {
    runtime2.log("\uD83D\uDD25 FREEZE condition met. Executing freeze on all vaults.");
    runtime2.log("\uD83D\uDD12 GLOBAL GUARDIAN PAUSED");
    for (const v of vaultStates) {
      await routeExecution(runtime2, "FREEZE", v, price);
    }
    return;
  }
  runtime2.log(`Volatility stdDev=${volatility.stdDev.toFixed(2)} | High=${volatility.isHighVolatility}`);
  runtime2.log("----- SentinelX CRE Workflow Completed -----");
}
var configSchema = exports_external.object({
  schedule: exports_external.string(),
  chainA: exports_external.object({
    selector: exports_external.number(),
    vaults: exports_external.array(exports_external.string()),
    guardian: exports_external.string(),
    stable: exports_external.string(),
    authority: exports_external.string()
  }),
  chainB: exports_external.object({
    selector: exports_external.number(),
    executor: exports_external.string()
  }),
  custodians: exports_external.array(exports_external.object({
    name: exports_external.string(),
    endpoint: exports_external.string(),
    publicKey: exports_external.string()
  })),
  quorum: exports_external.number()
});
var cron = new CronCapability;
var onCronTrigger = async (runtime2, _payload) => {
  runtime2.log("SentinelX cycle starting...");
  await runWorkflow(runtime2);
  runtime2.log("SentinelX cycle completed.");
  return { status: "completed" };
};
function initWorkflow(config, _secrets) {
  return [
