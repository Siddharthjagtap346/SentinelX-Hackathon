...
function evaluateRiskPolicy(health, lastEscalation) {
  const thresholds = [180, 160, 140];
  if (health > thresholds[0])
    return "NONE";
  if (health <= thresholds[0] && lastEscalation === 0)
    return "PARTIAL";
  if (health <= thresholds[1] && lastEscalation === 1)
    return "FULL";
  if (health <= thresholds[2] && lastEscalation === 2)
    return "FREEZE";
  return "NONE";
}
function computeSystemicRisk(vaultStates) {
  const unhealthy = vaultStates.filter((v) => Number(v.health) < 150);
  const ratio = unhealthy.length / vaultStates.length;
  return {
    unhealthyCount: unhealthy.length,
    total: vaultStates.length,
    ratio,
    systemicFreeze: ratio > 0.5
  };
}
...
async function getVaultState(runtime2, selector, vault, price) {
  ...
  const [healthResp, escalationResp, nonceResp] = await Promise.all([
  ...
  const lastEscalation = decodeFunctionResult({
    abi: [
      {
        name: "lastEscalation",
        type: "function",
...
async function getReserveHealth(runtime2) {
  const http = new cre.capabilities.HTTPClient;
  const reservePercent = await http.sendRequest(runtime2, (sendRequester) => {
    const response = sendRequester.sendRequest({
    ...
    const data = json(response);
    return Number(data.reservePercent);
  }, consensusMedianAggregation());
  const value2 = await reservePercent().result();
...
async function routeExecution(runtime2, action, vaultState, price) {
  ...
  logger.info("Nonce verification", {
    vault,
  ...
  try {
    ...
    logger.info("Report submitted to DON", {
      action,
      vault,
...
async function runWorkflow(runtime2) {
  ...
  const reserveFreeze = reserveHealth < 100;
  runtime2.log(`Reserve health: ${reserveHealth}%`);
  const vaultStates = [];
  for (const vault of runtime2.config.chainA.vaults) {
    ...
    runtime2.log(`[Vault ${vault}] Health=${state.health} | Escalation=${EscalationOrder[state.lastEscalation]} | Nonce=${state.vaultNonce}`);
    vaultStates.push({
      vault,
      ...state
    });
  }
  const systemic = computeSystemicRisk(vaultStates);
  runtime2.log(`Systemic Risk Ratio=${systemic.ratio.toFixed(2)} | Freeze=${systemic.systemicFreeze}`);
  const triggerFreeze = systemic.systemicFreeze || reserveFreeze;
  if (triggerFreeze) {
  ...
  for (const v of vaultStates) {
    let action = evaluateRiskPolicy(Number(v.health), v.lastEscalation);
    if (volatility.isHighVolatility && action === "NONE") {
      action = "WARNING";
    }
    if (action === "NONE")
      continue;
    await routeExecution(runtime2, action, v, price);
  }
